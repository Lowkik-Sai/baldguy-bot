    ...
    class ExtendedMessage extends Message {
        constructor(client, data, channel) {
            ...
            this.argsMissing = (msg, reason, cmd) => {
                const usage = cmd.info.usage ? `**${msg.guild.prefix}**${cmd.info.usage.replace(new RegExp("{prefix}", "g"), ` ** $ {
                    msg.guild.prefix
                } ** `)}` : "No usage provided.";
                const example = cmd.info.example ? `**${msg.guild.prefix}**${cmd.info.example.replace(new RegExp("{prefix}", "g"), ` ** $ {
                    msg.guild.prefix
                } ** `)}` : "No example provided.";
