...
class StatsCommand extends Command {
    ...
    exec(client, message, args) {
        let owner, uptime;
        require("child_process").exec("uptime -p", (_, out) => { uptime = out });
        client.config.owners.forEach(own => { owner = []; const fetch = client.users.resolve(own); owner.push(fetch) });
...
\`\`\`
        `)
        .addField("📌 Additional", `
Feel free to contribute this bot : https://github.com/zealcordNation/baldguy-bot
