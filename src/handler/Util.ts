...
export default class Util {
    ...
    constructor(client: BaldClient) {
        this.getAvatar = (user: any | Discord.UserResolvable) => {
            let isGif: string[] | boolean = client.users.resolve(user)!.displayAvatarURL().split('.');
            isGif = isGif[isGif.length - 1] === "gif";
            return client.users.resolve(user)!.displayAvatarURL({ format: isGif ? 'gif' : 'png' });
        }

        this.getGuildIcon = (guild: Guild) => {
